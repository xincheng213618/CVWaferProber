...
namespace CVWaferProber.ViewModels
{
    public class DieViewModel : ViewModelBase, IDisposable
    {
        ...
        public void InitTestProgressTimer()
        {
            ...
            if (logger.IsDebugEnabled)
                logger.DebugFormat("Die[{0}/{1}]进度定时器初始化完成，预测时长：{2}秒", MapX, MapY, _predictTestSeconds);
        }
            ...
            lock (_progressLock)
            {
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    MainViewModel.Instance?.DataMappingVM?.UpdateSingleDieProgress(
                        currentProgress,
                        $"已运行{_testElapsedSeconds}秒/预测{_predictTestSeconds}秒"
                    );
                });
            }
        }
        ...
        public void CompleteTestProgress()
        {
            lock (_progressLock)
            {
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    MainViewModel.Instance?.DataMappingVM?.UpdateSingleDieProgress(100, "测试完成");
                });
            }

            if (logger.IsDebugEnabled)
                logger.DebugFormat("Die[{0}/{1}]测试完成，进度拉满100%", MapX, MapY);
        }
        ...
        public void ChangeStatus(ChipStatus status, bool updateTime = false)
        {
            ...
            else
            {
                if (StartTestTime.HasValue && EndTestTime.HasValue)
                {
                    ...
                    TotalTime = span?.TotalSeconds.ToString("F1") + "秒";
    ...
    //{
    //    #region
    //    // 进度更新定时器（单Die专属，避免全局定时器冲突）
    //    private Timer _testProgressTimer;
    //    // 预测测试时长（秒）：从历史获取或默认240秒
    //    private int _predictTestSeconds;
    //    // 测试已运行时长（秒）：用于计算进度
    //    private int _testElapsedSeconds;
    //    // 进度更新锁：避免多线程同时更新进度
    ...
    //    /// 初始化测试进度定时器
    //    /// </summary>
    //    /// <param name="predictTestSeconds">预测测试时长（秒）</param>
    //    public void InitTestProgressTimer(int predictTestSeconds)
    //    {
    //        // 先停止原有定时器（防止重复启动）
    //        StopTestProgressTimer();
    ...
    //        lock (_progressLock)
    //        {
    //            _predictTestSeconds = predictTestSeconds;
    //            _testElapsedSeconds = 0;
    //            _isTesting = true;

    //            // 初始化定时器：1秒执行一次（1000毫秒）
    //            _testProgressTimer = new Timer(1000);
    //            _testProgressTimer.Elapsed += OnProgressTimerElapsed;
    //            _testProgressTimer.AutoReset = true; // 循环执行
    //            _testProgressTimer.Start();
    //        }
    //    }
    ...
    //            // 跨线程更新UI：通过Dispatcher同步到WPF主线程
    //            Application.Current.Dispatcher.Invoke(() =>
    //            {
    //                // 通知MappingDataViewModel更新单个Die进度
    //                MainViewModel.Instance?.DataMappingVM?.UpdateSingleDieProgress(currentProgress,
    //                    $"已运行{_testElapsedSeconds}秒/预测{_predictTestSeconds}秒");
    //            });
    //        }
    //    }
    ...
    //    private double CalculateTestProgress()
    //    {
    //        if (_predictTestSeconds <= 0) return 0;
    ...
    //            progress = (_testElapsedSeconds / (double)_predictTestSeconds) * 90;
    //        }
    //        // 阶段2：90%后 降速推进（剩余10%分配到剩余时间的2倍，避免提前到100%）
    //        else
    //        {
    //            double extraSeconds = _testElapsedSeconds - ninetyPercentSeconds;
    //            double extraTotalSeconds = _predictTestSeconds * 0.1 * 2; // 剩余10%时间放大2倍
    //            progress = 90 + (extraSeconds / extraTotalSeconds) * 10;
    //        }

    //        // 强制限制进度在0~99%之间，仅测试完成时手动拉满100%
    //        return Math.Max(0, Math.Min(99, progress));
    //    }

    //    // 新增：测试完成时停止定时器并拉满进度到100%
    //    public void CompleteTestProgress()
    //    {
    //        lock (_progressLock)
    //        {
    ...
    //        FirePropertyChanged();
    //        // 新增：初始化进度定时器，获取预测时长
    //        int predictSeconds = TestTimePredictService.GetPredictTestSeconds();
    //        InitTestProgressTimer(predictSeconds);
    //    }
    //    public void UnSelected()
    //    {
